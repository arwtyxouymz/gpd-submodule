...
void Learning::extractGraspsAndImages(
...
std::vector<cv::Mat>
Learning::createImages(const CloudCamera &cloud_cam,
  ...
  *cloud = *cloud_cam.getCloudProcessed();
  ...
  PointList point_list(points, cloud_cam.getNormals(),
                       cloud_cam.getCameraSource(), cloud_cam.getViewPoints());
  ...
  if (remove_plane_) {
    ...
    if (inliers->indices.size() > 0) {
      ...
      if (indices.size() > 0) {
        ...
        printf(
            "Removed plane from point cloud. Remaining points in cloud: %d.\n",
            (int)cloud->size());
        std::cout << point_list.getPoints().col(0).transpose() << "\n"
                  << cloud->at(0).getVector3fMap().transpose() << "\n";
      } else {
        printf("No points remaining after plane is removed! Using entire point "
               "cloud ...\n");
      }
    }
  }
...
#pragma omp parallel for private(nn_indices, nn_dists) num_threads(num_threads_)
  ...
  std::cout << "time for computing " << nn_points_list.size()
            << " point neighborhoods with " << num_threads_
            << " threads: " << omp_get_wtime() - t0_total << "s\n";

  if (image_params_.num_channels_ == 1 ||
      image_params_.num_channels_ ==
          3) // 3 channels image (only surface normals)
  {
    return createImages1or3Channels(hand_set_list, nn_points_list, is_valid,
                                    image_dims);
  } else if (image_params_.num_channels_ == 15) // 15 channels image
  {
    return createImages15Channels(hand_set_list, nn_points_list, is_valid,
                                  image_dims);
  }
  ...
  *cloud = *cloud_cam.getCloudProcessed();
  ...
  if (remove_plane_) {
    ...
    if (inliers->indices.size() > 0) {
      ...
      if (indices.size() > 0) {
        extract.filter(*cloud);
        sensor_msgs::PointCloud2 pc_ros_msg;
        pcl::toROSMsg(*cloud, pc_ros_msg);
        pc_ros_msg.header.frame_id = "head_rgbd_sensor_rgb_frame";
        pc_pub.publish(pc_ros_msg);
...
std::vector<cv::Mat>
Learning::createImages1or3Channels(const std::vector<GraspSet> &hand_set_list,
                                   const std::vector<PointList> &nn_points_list,
                                   const bool *is_valid,
                                   const Eigen::Vector3d &image_dims) const {
  double t0_images = omp_get_wtime();
  std::vector<cv::Mat> images(
      hand_set_list.size() * num_orientations_,
      cv::Mat(60, 60, CV_8UC(image_params_.num_channels_), cv::Scalar(0.0)));
...
cv::Mat Learning::createImage1or3Channels(const PointList &point_list,
                                          const Grasp &hand) const {
  ...
  Eigen::VectorXi cell_indices =
      GraspImage::findCellIndices(points_normals.first);
  ...
  if (is_plotting_)
  {
    std::string title = "Grasp Image (" +
  boost::lexical_cast<std::string>(image_params_.num_channels_);
...
std::vector<cv::Mat>
Learning::createImages15Channels(const std::vector<GraspSet> &hand_set_list,
                                 const std::vector<PointList> &nn_points_list,
                                 const bool *is_valid,
                                 const Eigen::Vector3d &image_dims) const {
  ...
  std::vector<cv::Mat> images(hand_set_list.size() * num_orientations_,
                              cv::Mat(60, 60, CV_8UC(15), cv::Scalar(0.0)));
  int num_images = 0;
...
#pragma omp parallel for num_threads(num_threads_)
  ...
  for (int i = 0; i < nn_points_list.size(); i++) {
    if (is_valid[i]) {
      Eigen::Matrix3Xd shadow =
          hand_set_list[i].calculateShadow4(nn_points_list[i], shadow_length);
      ...
      for (int j = 0; j < hands.size(); j++) {
        if (hand_set_list[i].getIsValid()(j)) {
          const int idx = i * num_orientations_ + j;
          images[idx] =
              createImage15Channels(nn_points_list[i], shadow, hands[j]);
          num_images++;
        }
      }
    }
  ...
  std::cout << "time to calculate " << num_images << " images with "
            << num_threads_ << " threads: " << omp_get_wtime() - t0_shadows
            << "s\n";
  return images;
  ...
  //    if (is_valid[i])
  //    {
  //      shadows[i] = hand_set_list[i].calculateShadow4(nn_points_list[i],
  //      shadow_length);
  //    }
  //  }
  //
  //  std::cout << "time to calculate " << nn_points_list.size() << " shadows
  //  with " << num_threads_ << " threads: " << omp_get_wtime() - t0_shadows <<
  //  "s\n";
  //
  ...
  //        {
  //          const int idx = i * num_orientations_ + j;
  //          images[idx] = createImage15Channels(nn_points_list[i], shadows[i],
  //          hands[j]);
  //          num_images++;
  //        }
  //      }
  ...
  //
  //  std::cout << "time to calculate " << num_images << " images with " <<
  //  num_threads_ << " threads: " << omp_get_wtime() - t0_images << "s\n";
  ////  std::cout << "==> total time to create images: " << omp_get_wtime() -
  ///t0_total << "s\n";
  //
  //  return images;
}

cv::Mat Learning::createImage15Channels(const PointList &point_list,
  ...
  GraspImage15Channels grasp_image(
      image_params_.size_, is_plotting_, &points_normals.first,
      &points_normals.second, &cropped_shadow_points);
  cv::Mat image = grasp_image.calculateImage();
  return image;
}

Matrix3XdPair Learning::transformToUnitImage(const PointList &point_list,
                                             const Grasp &hand) const {
  // 1. Transform points and normals in neighborhood into the hand frame.
  Matrix3XdPair points_normals = transformToHandFrame(
      point_list, hand.getSample(), hand.getFrame().transpose());
  ...
  points_normals.second =
      EigenUtils::sliceMatrix(points_normals.second, indices);

  return points_normals;
}

std::vector<int>
Learning::findPointsInUnitImage(const Grasp &hand,
...
Eigen::Matrix3Xd
Learning::transformPointsToUnitImage(const Grasp &hand,
...
Matrix3XdPair
Learning::transformToHandFrame(const PointList &point_list,
                               const Eigen::Vector3d &centroid,
                               const Eigen::Matrix3d &rotation) const {
  Matrix3XdPair pair;
  pair.first =
      rotation * (point_list.getPoints() -
                  centroid.replicate(1, point_list.getPoints().cols()));
